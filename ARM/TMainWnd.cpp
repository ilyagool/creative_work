...
TMainWnd::TMainWnd(QWidget *parent) : QMainWindow(parent)
  ...
  connect(exportBtn,&QPushButton::clicked,this,&TMainWnd::exportToCsv);
...
void TMainWnd::exportToCsv()
  {
  QModelIndexList items=categoryTable->selectionModel()->selectedRows();
  QString fileName=QFileDialog::getSaveFileName(this,tr("Save CSV file"),qApp->applicationDirPath(),tr("CSV files (*.csv)"));
  if (fileName.isEmpty())
    return;
  ...
  QStringList strings;
  QModelIndexList selected=categoryTable->selectionModel()->selectedRows();
  QString category=selected.size() ? m_categoryModel->index(selected.first().row(),m_categoryModel->record().indexOf("category")).data().toString() : tr("All categories");
  stream<<tr("Products for Category %1").arg(category)<<"\n"<<"\n";
  for (int i=0;i<m_productModel->columnCount();i++)
    strings<<m_productModel->headerData(i,Qt::Horizontal).toString();
  ...
  for (int i=0;i<m_productModel->rowCount();i++)
    {
    QSqlRecord record=m_productModel->record(i);qDebug()<<record;
    for (int j=0;j<m_productModel->columnCount();j++)
      strings<<record.value(j).toString();
